...
function PersonalLogin() {//学生登录
    const [value, setValue] = useState<IOnFinishProps>({ "password": "", "remember": false, "personID": "" });

    function onFinish() {//点击登录，提交表单
        console.log('Received values of form: ', value);
    };

    function onChangePassword(e: React.ChangeEvent<HTMLInputElement>) {
        setValue({ "password": e.target.value, "remember": value.remember, "personID": value.personID })
    }
    function onChangePersonID(e: React.ChangeEvent<HTMLInputElement>) {
        setValue({ "password": value.password, "remember": value.remember, "personID": e.target.value })
    }
    function isRemember(e: CheckboxChangeEvent) {
        setValue({ "password": value.password, "remember": e.target.checked, "personID": value.personID })
    }
    return (
        <Form
            name="normal_login"
        ...
        <>
            <Layout className="Login-layout">
                ...
                <Tabs type="card" defaultActiveKey="1" onChange={callback} className="LoginPannel">
                    <TabPane tab="本人登录" key="1">
                        <PersonalLogin />
                    </TabPane>
                    <TabPane tab="亲朋登录" key="2">
                        <FriendsLogin />
                    </TabPane>
